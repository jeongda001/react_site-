...
export default function BestCom(props) {
  ...
  return(
    ...
    <div className="detail_top">
      <div className="detail_img">
        <img src={Best[id].image} alt=""/>
      </div>

      <div className="detail_right">
        <p className="detail_title">[당일발송] {Best[id].title} </p>
        <p className="detail_txt span1">판매가 <span> {Best[id].price}</span>원</p>
        ...
        <p className="total">TOTAL : <span>{count * Best[id].price}</span>원</p>
      
        <button className="btn_buy">구매하기</button>
        <div className="btn_cart">
          <button onClick={() => {
              dispatch(addItem({id: Best[id].id, title: Best[id].title, price: Best[id].price, image: Best[id].image, count: count}))
              setPopup(true)
            }}>장바구니</button>
          <button>찜하기</button>
        </div>
    ...
    <div className="detail_bottom">
      ...
      <div className="explain_img">
        <img src={Bestdetail[id].image} alt="" />
      </div>
    </div>
    <ScrollToTop/>
